...
function buildMiniMaxJsonInstruction() {
  ...
  return [
    ...
    "不要改 key 名，不要输出中文 key，不要增加 schema 之外的 top-level key。",
    "stage enum: orientation, exploring, structuring, configuration_ready, handoff_ready.",
    "recommendedNextAction enum: continue_orientation, compare_options, ask_one_question, enter_configurator, submit_inquiry, handoff_to_operator.",
    "extractedFacts.scaleBand enum: small, medium, large, undetermined.",
    "budgetUnderstanding.level enum: unknown, rough_range, service_tradeoff, ready_for_estimate.",
    "safetyFlags[].code enum: quote_requested, availability_requested, supplier_internal_requested, mock_content_risk, private_data_risk.",
    "没有值的 optional 字段用 null；列表用 []；不要省略 top-level required keys。",
    "followupQuestion、budgetUnderstanding、opsOnlySummary、leadSignals 没有值时必须填 null。",
    "requestedServices、missingFacts、safetyFlags 没有值时必须填 []。",
